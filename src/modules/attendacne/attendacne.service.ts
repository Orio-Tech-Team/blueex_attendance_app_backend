...
export class AttendacneService {
  ...
  async getAttendanceDataDto(getAttendanceDataDto: GetAttendanceDataDto) {
    let query = `SELECT att.*,ep.employee_name,ep.shift_id,s.start_time,s.end_time, (att.outtime-att.intime) as working_hours FROM attendance att left join employees ep on ep.employee_number=att.employee_number left join shifts s on s.id=ep.shift_id WHERE att.attendance_date BETWEEN ${getAttendanceDataDto.from_date}.00:00:00 AND ${getAttendanceDataDto.to_date}.23:59:59;`;
